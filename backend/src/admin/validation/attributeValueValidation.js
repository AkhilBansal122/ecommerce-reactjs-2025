...
module.exports = {
    createAttributeValueValidation: async (req, res, next) => {
        // Define the validation schema using Joi
        const schema = Joi.object({
            name: Joi.string()
                .min(1) // Ensures the name is at least 3 characters long (adjustable based on requirements)
                .required()
                .messages({
            ...
            attribute_id: Joi.string()
                .custom((value, helpers) => {
                    ...
                    if (!mongoose.Types.ObjectId.isValid(value)) {
                        return helpers.message('Main Attribute ID must be a valid MongoDB ObjectId.');
                    }
                    return value; // Valid ObjectId, return it
                })
                .required()
                .messages({
                    'any.required': 'Main Attribute ID is required.',
                }),
        });

        validationRequest(req, res, next, schema);
    },
    updateAttributeValueValidation: async (req, res, next) => {
        // Define the validation schema using Joi
        const schema = Joi.object({
            id: Joi.string()
            ...
            name: Joi.string()
                .min(1) // Ensures the name is at least 3 characters long
                .required()
                .messages({
                    'string.min': 'Name must be at least 1 characters long.',
                    'any.required': 'Name is required.',
    ...
    deleteAttributenValidation: async (req, res, next) => {
        // Define the validation schema using Joi
        const schema = Joi.object({
            id: Joi.string()
                .length(24) // Ensures the id is exactly 24 characters long (for MongoDB ObjectId)
    ...
    listAttributeValueValidation: async (req, res, next) => {
        // Define the validation schema using Joi
        const schema = Joi.object({
            page: Joi.number()
    ...
    statusChanngeAttributeValueValidation: async (req, res, next) => {
        // Define the validation schema using Joi
        const schema = Joi.object({
            id: Joi.string()
                .length(24) // Ensures the id is exactly 24 characters long (for MongoDB ObjectId)
