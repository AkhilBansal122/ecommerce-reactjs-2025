...
import { categoryAdd, getCategory, categoryUpdate, categoryStatusChange } from "../../services/category";
...
export const Category = () => {
  ...
  const getCategoryData = async (pageClick, perPage) => {
    try {
      const config = localStorage.getItem("jwtToken");
      const result = await getCategory({ page: pageClick, limit: perPage }, config);
      if (result?.status) {
  ...
  const onSubmit = async () => {
    ...
    if (data.name.length > 0) {
      ...
      const result = data.id.length === 0 ? await categoryAdd(data, config) : await categoryUpdate(data, config);
