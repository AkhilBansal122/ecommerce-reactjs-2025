...
import { CommonPostAction, UserStatusUpdateAction  , PrivacyAndPolicyDeleteAction} from '../../../features/CommonSlice';
...
const PrivacyPolicyList = () => {
    ...
    const privacyAndPolicyListing = () => {
        setLoader(true)
        CommonPostAction("/privacy-policy-list", { limit: perPageCount, offset: (currentPage - 1) * perPageCount }, (response) => {
    ...
    const submitUpdatePrivacyPolicy = (data) =>{
        if (data.target[0].value && data.target[1].value) {
            CommonPostAction("/update-privacy-policy", {
                ...
                if (response.status) {
                    ...
                    privacyAndPolicyListing();
                    ToastOverSuccess("Privacy Policy updated successfully");
                } else {
                    ToastOverError(response.message);
                }
            })();
    ...
    const UpdatePrivacyPolicyModal = () =>{
    console.log(selectedPrivacyPolicy?.title , "selectedPrivacyPolicy")
        return <div className='delete_modal_main'>
      
        <div className='deleteNote'> Edit Privacy Policy </div>
        <form className='responsive_common'
            onSubmit={(e) => {
                e.preventDefault()
                submitUpdatePrivacyPolicy(e)
            }}
        >
            <div className='row g-4'>
                <div className='col-12'>
                    <label className='commmon_label' > Title</label>
                    <input className="form-control" type="text" name="code_name" defaultValue={selectedPrivacyPolicy?.title} />
                </div>
                <div className='col-12'>
                    <label className='commmon_label'>Description</label>
                    <textarea className="form-control themeScrollbar" name="description" cols="3" rows="5" defaultValue={selectedPrivacyPolicy?.description}></textarea>
                </div>                    
            </div>
            <div className='tow_button_card'>
                <button className='themeBtn w-auto m-0' type='button' onClick={() => setUpdatModalOpen(false)}> Cancel</button>
    ...
    const submitDeletePolicy = () => {
        PrivacyAndPolicyDeleteAction({ id: selectedPrivacyPolicy.id }, (response) => {
        ...
        <>
            <section className='mainSection'>
                ...
                <div className='title_breadcrumb_section'>
                    <div className='title_page'>Privacy Policy List</div>
                    <nav aria-label="breadcrumb">
                        <ol className="breadcrumb">
                            <li className="breadcrumb-item"><Link to="/admin/dashboard">Home</Link></li>
                            <li className="breadcrumb-item active" aria-current="page">Privacy Policy list</li>
                        </ol>
                    </nav>
                </div>
